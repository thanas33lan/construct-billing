...
$rs_field = 'Product Name,HSN Code,Quantities,Minimum Quantities,Price,Tax,Last Paid,Product Status,Product Added On';
...
if(isset($rs_field) && trim($rs_field)!= ''){
    ...
    $expenseList = $db->rawQuery($_SESSION['productQuery']);
      $output = array();
      foreach($expenseList as $aRow){
        ...
        $row[] = ucwords($aRow['product_status']);
        $row[] = date('d-M-Y h:i:s \P\M', strtotime($aRow['product_added_on']));;
        $output[] = $row;
      }
     ...
     $filename = 'Product-report-' . date('d-M-Y-H-i-s') . '.xlsx';
