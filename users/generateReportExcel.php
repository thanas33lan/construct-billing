...
$rs_field = 'User Name,Login Id,Contact Number,User Created On,User Status';
...
if(isset($rs_field) && trim($rs_field)!= ''){
    ...
    $expenseList = $db->rawQuery($_SESSION['userQuery']);
      $output = array();
      foreach($expenseList as $aRow){
        ...
        $row[] = $aRow['login_id'];
        $row[] = $aRow['user_phone'];
        $row[] = date('d-M-Y h:i:s \P\M', strtotime($aRow['user_added_on']));
     ...
     $filename = 'User-report-' . date('d-M-Y-H-i-s') . '.xlsx';
