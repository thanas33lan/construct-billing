...
$rs_field = 'Supplier Name,Supplier Address,Email,Contact Number,Alternate Contact Number,Supplier Status';
...
if(isset($rs_field) && trim($rs_field)!= ''){
    ...
    $expenseList = $db->rawQuery($_SESSION['supplierQuery']);
      $output = array();
      foreach($expenseList as $aRow){
        $row = array();
        $row[] = ucwords($aRow['supplier_name']);
        $row[] = $aRow['supplier_address'];
        $row[] = $aRow['supplier_email'];
