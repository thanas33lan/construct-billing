...
$pdf->setHeading('', ucwords($cResult[0]['company_name']),$cResult[0]['company_logo']);

$pdf->SetCreator(PDF_CREATOR);
$pdf->SetAuthor($cResult[0]['company_name']);
$pdf->SetTitle($cResult[0]['company_name'].'- Buyer History');
$pdf->SetSubject($cResult[0]['company_name'].'-Buyer History-'.ucwords($cliResult[0]['client_name']));
...
$html .= '<table style="padding:3px;">';
    ...
    $html .= '<tr>';
        $html .= '<td style="padding:2px;line-height:15px;">';
            ...
            $html .= '<span>Mobile Number: '.$cResult[0]['company_phone'].'</span><br/>';
            $html .= '<span>GSTIN: '.$cResult[0]['gst_number'].'</span><br/>';
            $html .= '<span>E-Mail: '.$cResult[0]['company_email'].'</span><br>';
        $html .= '</td>';
    $html .= '</tr>';
    ...
    $html .='<tr>';
        $html .='<td>';
            ...
            if(isset($cliResult[0]['client_mobile_no']) && trim($cliResult[0]['client_mobile_no']) != ''){
                $html .='<span>Mobile Number: '.$cliResult[0]['client_mobile_no'].'</span><br/>';
            }
            if(isset($cliResult[0]['gst_no']) && trim($cliResult[0]['gst_no']) != ''){
                $html .='<span>GSTIN: '.$cliResult[0]['gst_no'].'</span><br/>';
            }
            if(isset($cliResult[0]['client_email_id']) && trim($cliResult[0]['client_email_id']) != ''){
...
$html .= '<br/><table border="1" style="padding:3px;">
            ...
            $html .= '<td><b>'.number_format(($totalPendingForPay-$totalCollection),2).'</b></td>';
            $html .= '<td><b>'.number_format($totalCollection,2).'</b></td>';
            $html .= '</tr>';
...
$pdf->writeHTML($html);
$pdf->lastPage();
$filename = 'voucher' . date('d-M-Y-H-i-s') . '.pdf';
