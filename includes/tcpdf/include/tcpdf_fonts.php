...
class TCPDF_FONTS
{
	 ...
	 * @public static
	 */
	public static function unichr(int $c, bool $unicode = true): string
	{
		if (!$unicode) {
			return chr($c);
		}
	 ...
	 * @public static
	 */
	public static function UTF8ArrSubString(array $strarr, $start = 0, $end = null, bool $unicode = true): string
	{
		$start = is_numeric($start) ? (int)$start : 0;
		$end = is_numeric($end) ? (int)$end : count($strarr);

		$string = '';
		for ($i = $start; $i < $end && isset($strarr[$i]); ++$i) {
			$string .= self::unichr((int)$strarr[$i], $unicode); // Cast to int here
		}
